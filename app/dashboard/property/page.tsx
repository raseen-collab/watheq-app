...
export default async function PropertyPage() {
  ...
  const { data: profile } = await supabase
    .from("profiles").select("org_name, billing_name, vat_number, cr_number, billing_phone, plan, trial_ends_at, subscribed_until, due_soon_days, due_imminent_days").eq("id", user!.id).maybeSingle();
  ...
  return <PropertyView dueSoonDays={(profile as any)?.due_soon_days} dueImminentDays={(profile as any)?.due_imminent_days} initial={properties || []} orgName={profile?.org_name || ""}
